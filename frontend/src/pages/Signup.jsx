...
//     setLoading(true);
//     try {
//       const res = await axios.post('https://0011-2409-40c2-5054-2e1-3558-d96-e3b6-65c4.ngrok-free.app/api/auth/signup', form);
...
//         <h2 style={{ textAlign: 'center', marginBottom: 24, color: '#1e293b' }}>Sign Up</h2>
...
//           <Link to="/signin" style={{ color: '#3b82f6', textDecoration: 'underline', fontWeight: 500 }}>Sign In</Link>
//         </div>
//       </div>
//     </div>
//   );
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await axios.post(
        'https://ef24-2409-40c2-5054-2e1-3558-d96-e3b6-65c4.ngrok-free.app/api/auth/signup',
        form
      );
      alert("Signup successful");
    ...
    <div style={{ minHeight: '100vh', background: 'linear-gradient(135deg, #e0e7ff 0%, #f8fafc 100%)', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
      ...
      <div style={{ background: '#fff', padding: 32, borderRadius: 16, boxShadow: '0 4px 24px rgba(0,0,0,0.08)', minWidth: 340 }}>
        ...
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
          <label style={{ fontWeight: 500, color: '#334155' }}>
            Name
            <input required onChange={(e) => setForm({ ...form, name: e.target.value })} style={{ width: '100%', padding: 10, marginTop: 6, borderRadius: 8, border: '1px solid #cbd5e1', fontSize: 16 }} />
          </label>
          <label style={{ fontWeight: 500, color: '#334155' }}>
            Email
        ...
        <div style={{ textAlign: 'center', marginTop: 18, color: '#64748b' }}>
          Already have an account? <Link to="/signin" style={{ color: '#3b82f6', textDecoration: 'underline', fontWeight: 500 }}>Sign In</Link>
        </div>
      </div>
    </div>
  );
